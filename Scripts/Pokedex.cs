...
//this is used to call generic pokemon information
public class Pokedex : MonoBehaviour
{
    public static List<int> bannedPokes = new List<int>() { 10, 11, 13, 14, 94, 129, 132, 150, 151, 175, 235, 249, 250, 251, 266, 268, 374};
    public static List<string> allowedPokeTypes = new List<string>() { "dark", "normal", "fighting", "flying", "poison", "ground", "rock", "bug", "ghost", "steel", "fire", "water", "grass", "electric", "psychic", "ice", "dragon" };
    public static Dictionary<int, string> pokeSpeciesByNum = new Dictionary<int, string>();
    public static Dictionary<string, List<int>> pokeNumsByType = new Dictionary<string, List<int>>();
    public static Dictionary<string,Poke> pokes = new Dictionary<string, Poke>();
    public static Dictionary<string,Learnset> learnsets = new Dictionary<string,Learnset>();
    public TextAsset LearnsetFile;
    public TextAsset PokedexFile;
    ...
    void Start()
    {
        //var jsonTextFile = Resources.Load<TextAsset>("Scripts/scaryjson/learnset");
        //Then use JsonUtility.FromJson<T>() to deserialize jsonTextFile into an object

        GetLearnsets(LearnsetFile.ToString());
        GetPokedex(PokedexFile.ToString());
        CreatePokeTypeLists();
    }

    private void GetLearnsets(string jsfile)
    {
    ...
    private void GetPokedex(string jsfile)
    {
        SimpleJSON.JSONNode data = SimpleJSON.JSON.Parse(jsfile);
        ...
        foreach (SimpleJSON.JSONNode info in data)
        {
            ...
            if (poNum != 0)
            {
                ...
                {
                    Debug.Log("CANT ADD ANOTHER " + poSpecies);
                }
                else
                {
            ...
            {
                Debug.Log("TRIED TO CREATE MISSINGNO. BAD BAD BAD");
            }
        }
    }
    ...
    {
        int r = UnityEngine.Random.Range(1, 377);
        Poke randompoke;
        while (bannedPokes.Contains(r) || !pokeSpeciesByNum.ContainsKey(r))
        {
            r = UnityEngine.Random.Range(1, 377);
        }
        if (!Pokedex.pokeSpeciesByNum.ContainsKey(r))
        {
            Debug.Log("THERE IS NO POKEMON WITH THE NUMBER " + r);
        }
    ...
    public Poke GetRandomPokeOfType(string pType)
    {
        ...
        Debug.Log(tests);
        int r = UnityEngine.Random.Range(0, typeNums.Count);
        ...
        Debug.Log("CHOSEN POKEMON WAS " + randompoke.Species);
        return randompoke;
    }

    public void CreatePokeTypeLists()
    {
